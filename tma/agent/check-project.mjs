...
const localComponentName = (root, path) => {
    ...
    if (relativePath.includes("/components/tma/")) return undefined
    return fileName === "index" ? directoryName : fileName
}

const isManagedTmaComponent = (root, path) =>
    relative(root, path).replaceAll("\\", "/").includes("/components/tma/")

const directTmaImports = (source) =>
    [...source.matchAll(/import\s*\{([^}]+)\}\s*from\s*["']@deslop\/tma["']/g)]
...
export async function checkProject({ root = process.cwd() } = {}) {
    ...
    if (!(await exists(resolve(projectRoot, "src/components/tma/index.js")))) {
        failures.push(
            "local TMA components are missing; run npx @deslop/tma setup"
        )
    }
    ...
    if (!("@deslop/tma" in dependencies)) {
        failures.push("@deslop/tma is not listed in project dependencies")
    }

    for (const dependency of Object.keys(dependencies)) {
        if (forbiddenDependency.test(dependency)) {
            failures.push(
                `package.json includes ${dependency}; use @deslop/tma instead`
            )
        }
    }
    ...
    const productSourceFiles = sourceFiles.filter(
        (path) => !isManagedTmaComponent(projectRoot, path)
    )
    ...
    if (hasInterface && !allSource.includes("components/tma")) {
        failures.push(
            "interface code does not import local components from src/components/tma"
        )
    }
    if (hasInterface && !allSource.includes("@deslop/tma/styles.css")) {
        failures.push(
            'TMA styles are not connected; import "@deslop/tma/styles.css" once in the app entry'
        )
    }
    if (hasInterface && !allSource.includes("TMAProvider")) {
        failures.push("the application is not wrapped in TMAProvider")
    }

    for (const path of sourceFiles) {
        const source = sources.get(path)
        const file = relative(projectRoot, path).replaceAll("\\", "/")
        if (isManagedTmaComponent(projectRoot, path)) continue
        const duplicate = localComponentName(projectRoot, path)

        if (duplicate && componentNames.has(duplicate.toLowerCase())) {
            failures.push(
                `${file} duplicates the TMA ${duplicate} component; import it from src/components/tma`
            )
        }
        if (forbiddenImport.test(source)) {
            failures.push(`${file} imports another UI or icon library`)
        }
        if (/from\s+["']@deslop\/tma\/(?:dist|src)\//.test(source)) {
            failures.push(
                `${file} imports a private TMA path; use the public package export`
            )
        }
        const directComponents = directTmaImports(source)
        if (directComponents.length > 0) {
            failures.push(
                `${file} imports ${directComponents.join(", ")} directly from @deslop/tma; use src/components/tma`
            )
        }
        if (rawControl.test(source)) {
            failures.push(
                `${file} renders a raw control; use the matching TMA component`
            )
        }
        ...
        if (inlineStyle.test(source)) {
            failures.push(
                `${file} uses inline styles; use TMA and Primitives styles`
            )
        }
