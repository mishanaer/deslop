...
const managedComponentBanner = "// @deslop/tma managed component"
const rulesStart = "<!-- deslop-tma:rules:start -->"
const rulesEnd = "<!-- deslop-tma:rules:end -->"
const hookStart = "# deslop-tma:check:start"
const hookEnd = "# deslop-tma:check:end"
...
const ensureAgentFiles = async (root) => {
    const target = resolve(root, ".deslop/tma")
    ...
    await writeFile(
        ...
        "# Исходники TMA\n\nЭто читаемая копия исходников для агентов. Не импортируй файлы из этой папки: рабочие компоненты находятся в `src/components/tma`.\n",
        "utf8"
    )
}

const ensureLocalComponents = async (root, { overwrite = false } = {}) => {
    const target = resolve(root, "src/components/tma")
    const conflicts = []
    const names = ["TMAProvider", ...catalog.components.map(({ name }) => name)]
    ...
    for (const name of names) {
        await writeManagedComponent(
            ...
            `${managedComponentBanner}\n// Run \`npx @deslop/tma setup\` to refresh this file.\nexport { ${name} as default, ${name} } from "@deslop/tma"\n`
        )
    }
...
const ensurePackageScripts = async (root) => {
    ...
    projectPackage.scripts["check:tma"] = "tma check"

    const hookCommand = "tma hook"
    ...
    const hasDependency = "@deslop/tma" in dependencies
    projectPackage.dependencies ??= {}
    if (!hasDependency) {
        projectPackage.dependencies["@deslop/tma"] =
...
const ensureEntrySetup = async (root) => {
    ...
    if (!entryPath) {
        return [
            'Entry file was not detected; import "@deslop/tma/styles.css" and wrap the app in TMAProvider manually',
        ]
    }

    let source = await readFile(entryPath, "utf8")
    const imports = []
    const gatewayPath = resolve(root, "src/components/tma/index.js")
    ...
    if (!source.includes("@deslop/tma/styles.css")) {
        imports.push('import "@deslop/tma/styles.css"')
    }

    if (!source.includes("TMAProvider")) {
        const app = source.match(/<App\b[^>]*\/>/)
        if (app) {
            imports.push(`import { TMAProvider } from "${gatewayImport}"`)
            source = source.replace(
                app[0],
                `<TMAProvider>${app[0]}</TMAProvider>`
            )
        } else {
            if (imports.length > 0) {
            ...
            return ["Wrap the application root in TMAProvider"]
        }
    }
...
export async function installHook(root = process.cwd()) {
    ...
    const command = [
        "if [ ! -x node_modules/.bin/tma ]; then",
        '  echo "TMA check is unavailable. Install project dependencies." >&2',
        "  exit 1",
        "fi",
        "node_modules/.bin/tma check",
    ].join("\n")
...
export async function setupProject(
    ...
    const localPackage = resolve(
        projectRoot,
        "node_modules/@deslop/tma/package.json"
    )
    ...
    log("TMA setup completed:")
    log("- ready AGENTS.md rules are connected")
    log("- local components are available in src/components/tma")
    log("- readable component source is available in .deslop/tma/source")
    log("- component catalog is available in .deslop/tma")
    log("- npm run check:tma is configured")
    log(
        hook.installed
...
const printHelp = () => {
    console.log(`TMA project integration

Usage:
  tma setup              install local TMA components, rules, styles, and checks
  tma check              validate that the project uses only TMA UI
  tma hook               install or refresh the local pre-commit check
...
Options:
  ...
  --overwrite            replace conflicting local TMA component files`)
}
...
async function main() {
    ...
    if (command === "check") {
        ...
        console.log("TMA project rules passed.")
        return
    }
