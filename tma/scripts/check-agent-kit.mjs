...
import { setupProject } from "../bin/tma.mjs"
...
assert.equal(packageJson.bin.tma, "./bin/tma.mjs")
...
assert(rules.includes(".deslop/tma/COMPONENTS.md"))
...
const fixture = await mkdtemp(resolve(tmpdir(), "deslop-tma-agent-"))
...
import { Page, Text } from "./components/tma/index.js"
...
`,
    ...
    const localBin = resolve(fixture, "node_modules/.bin/tma")
    await writeFile(
        localBin,
        `#!/bin/sh\nexec "${process.execPath}" "${resolve(root, "bin/tma.mjs")}" "$@"\n`,
        "utf8"
    )
    ...
    const localPage = await readFile(
        resolve(fixture, "src/components/tma/Page.js"),
        "utf8"
    )
    const sourcePage = await readFile(
        resolve(fixture, ".deslop/tma/source/components/Page/index.js"),
        "utf8"
    )

    assert(projectRules.includes("Keep this rule."))
    assert.equal(projectRules.match(/deslop-tma:rules:start/g)?.length, 1)
    assert.equal(projectPackage.scripts["check:tma"], "tma check")
    assert.equal(projectPackage.scripts.prepare.match(/tma hook/g)?.length, 1)
    assert.equal(projectPackage.dependencies["@deslop/tma"], "^0.1.0")
    assert.equal(entry.match(/@deslop\/tma\/styles\.css/g)?.length, 1)
    assert.equal(entry.match(/import \{ TMAProvider \}/g)?.length, 1)
    assert(entry.includes("<TMAProvider><App /></TMAProvider>"))
    assert.equal(hook.match(/deslop-tma:check:start/g)?.length, 1)
    assert(hook.includes("node_modules/.bin/tma check"))
    ...
    assert.equal(
        await readFile(resolve(fixture, "src/components/tma/Page.js"), "utf8"),
        "// Product-owned Page component\n"
    )
...
`,
    ...
    assert(
        invalid.failures.some((failure) =>
            failure.includes("duplicates the TMA")
        )
    )
...
console.log("TMA consumer setup and checks are valid.")
